...
try:
    ...
    csv_output.write('repo_path,repo_name,repo_id,last_updated,last_commiter,most_commits,most_num,second_most,second_num\n')
except:
...
for index, row in by_repo.iterrows():
    basic_info = row.repo_path + ',' + row.repo_name + ',' + str(row.repo_id) + ',' + str(row.cmt_author_timestamp) + ',' + row.cmt_author_email + ','
